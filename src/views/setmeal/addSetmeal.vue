  ...
  <div class="addBrand-container">
    <div class="container">
      <el-form ref="ruleForm"
               ...
               class="demo-ruleForm">
        <div>
          <el-form-item label="セットメニュー名:"
                        prop="name">
            <el-input v-model="ruleForm.name"
                      placeholder="セットメニュー名を入力してください"
                      maxlength="14" />
          </el-form-item>
          <el-form-item label="セットメニューのカテゴリ:"
                        prop="idType">
            <el-select v-model="ruleForm.idType"
                       placeholder="セットメニューのカテゴリを選択してください"
                       @change="$forceUpdate()">
        ...
        <div>
          <el-form-item label="セットメニューの価格:"
                        prop="price">
            <el-input v-model="ruleForm.price"
                      placeholder="セットメニューの価格を設定してください" />
          </el-form-item>
        </div>
        <div>
          <el-form-item label="セットメニューの料理:"
                        required>
            <el-form-item>
              <div class="addDish">
        ...
        <div>
          <el-form-item label="セットメニューの画像:"
                        required
                        prop="image">
            <image-upload :prop-image-url="imageUrl"
                          @imageChange="imageChange">
              画像のサイズは2MBを超えないでください。<br>PNG、JPEG、JPG形式の画像のみアップロード可能です<br>200x200ピクセルまたは300x300ピクセルサイズの画像のアップロードをお勧めします
            </image-upload>
          </el-form-item>
        </div>
        <!-- <el-form-item
        ...
        <div class="address">
          <el-form-item label="セットメニューの説明:">
            <el-input v-model="ruleForm.description"
                      type="textarea"
                      :rows="3"
                      maxlength="200"
                      placeholder="セットメニューの説明，最長200文字" />
          </el-form-item>
        </div>
    ...
    <el-dialog v-if="dialogVisible"
      ...
      <span slot="footer"
        ...
        <el-button type="primary"
                   @click="addTableList">追加</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default class extends Vue {
  ...
  get rules() {
    return {
      name: {
        ...
        validator: (rule: any, value: string, callback: Function) => {
          if (!value) {
            ...
            const reg = /^([A-Za-z0-9\uFF10-\uFF19\uFF21-\uFF3A\uFF41-\uFF5A\u3040-\u309F\u30A0-\u30FF\uFF66-\uFF9D\u3400-\u4DBF\u4E00-\u9FFF]){2,20}$/
            if (!reg.test(value)) {
      ...
      idType: {
        required: true,
        message: 'セットメニューのカテゴリを選択してください',
        trigger: 'change'
      },
      image: {
        required: true,
        message: '料理の画像を入力してください'
      },
      price: {
        required: true,
        // 'message': '请输入套餐价格',
    ...
    ;(this.$refs[formName] as any).validate((valid: any) => {
      if (valid) {
        if (this.dishTable.length === 0) {
          return this.$message.error('セットメニューの料理が空です')
        }
        if (!this.ruleForm.image) return this.$message.error('セットメニューの画像が空です')
        ...
        if (this.actionType == 'add') {
          ...
          addSetmeal(prams)
            .then(res => {
              if (res && res.data && res.data.code === 1) {
                this.$message.success('セットメニューを追加しました！')
                if (!st) {
          ...
          editSetmeal(prams)
            .then(res => {
              if (res.data.code === 1) {
                this.$message.success('セットメニューを変更しました！')
                this.$router.push({ path: '/setmeal' })
              } else {
